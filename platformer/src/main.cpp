...
class Entity { 
  public: 
    ...
    virtual const sf::Sprite & getDrawable() = 0;   
};

struct PlayerProperties: public Properites { 
  ...
  void updateAnimations(float dt) { 
    if(current_direction == RIGHT) {
      sprite.setTextureRect(_anim_mgr.play(RIGHT,dt));
    }
    else if(_current_direction == LEFT) {
      sprite.setTextureRect(_anim_mgr.play(LEFT,dt));
    }
    else if(idle) { 
      if(current_direction == RIGHT) { 
...
class Player : public Entity { 
  ...
  virtual const sf::Sprite & getDrawable() { 
    return _properties.sprite; 
  }
};
